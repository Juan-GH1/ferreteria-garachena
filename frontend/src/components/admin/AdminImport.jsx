...
import { AlertCircle, AlertTriangle, Check, CheckCircle2, FileSpreadsheet, UploadCloud, X } from 'lucide-react';
import { ADMIN_KEY_STORAGE, importProducts } from '../../api';
...
export default function AdminImport() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black tracking-tight text-navy-900">Importar Catálogo</h1>
        <p className="text-sm text-slate-500 font-medium mt-1">Sincronización masiva de inventario · puente hacia Sisgen</p>
      </div>

      <div className="bg-white p-6 rounded-2xl shadow-soft ring-1 ring-slate-900/5 space-y-4">
        <div>
          <h2 className="text-base font-black tracking-tight text-navy-900 flex items-center gap-2">
          ...
          <p className="text-sm text-slate-500 mt-1">
            ...
            <code className="bg-slate-50 px-1 py-0.5 rounded">precio</code>, <code className="bg-slate-50 px-1 py-0.5 rounded">stock_total</code>. Si el
            SKU ya existe se actualiza precio y stock; si no, se crea el producto. Si el servidor exige clave de administrador, configúrala en la barra
            lateral.
          </p>
        </div>
      ...
      {sessionLog.length > 0 && (
        <div className="bg-white p-6 rounded-2xl shadow-soft ring-1 ring-slate-900/5">
          <h3 className="text-sm font-black text-slate-500 uppercase tracking-wide mb-3">Importaciones de esta sesión</h3>
